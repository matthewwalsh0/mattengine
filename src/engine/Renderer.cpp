...
void Renderer::init() {
	...
	float vertices[] = {-0.5f, -0.5f, -0.5f, 0.0f, 0.0f, -1.0f, 0.5f, -0.5f,
		...
		0.0f, -0.5f, -0.5f, -0.5f, -1.0f, 0.0f, 0.0f, -0.5f, -0.5f, -0.5f,
	...
	MATTENGINE_ASSERT(
		!ftError, "Could not iniailise Freetype - Error Code: %d", ftError);
}

void Renderer::drawQuad(const float x, const float y, const float width,
...
void Renderer::drawQuadGradient(const float x, const float y, const float width,
	...
	GLint colourStartLocation =
		glGetUniformLocation(m_shaderColour, "u_ColourStart");
	glUniform3f(
		colourStartLocation, colourStart.x, colourStart.y, colourStart.z);
...
void Renderer::drawQuadTexture(const float x, const float y, const float width,
	const float height, const Texture& texture, const unsigned int tileCount,
	float alpha) const {
	...
	GLint tileCountLocation =
		glGetUniformLocation(m_shaderTexture, "u_TileCount");
...
void Renderer::drawText(const float x, const float y, const float height,
	...
	for (char character : text) {
		...
		if (textureIt == m_charTextures.end()) {
			...
			MATTENGINE_ASSERT(!charError,
				"Could not load character: %c - Error Code: %d", character,
				charError);
			...
			float aspect =
				bitmapWidth == 0 ? 0.8 : bitmapWidth / (float)bitmapHeight;
...
void Renderer::drawQuad(const float x, const float y, const float width,
	...
	glm::mat4 transform =
		glm::translate(glm::mat4(1.0f), glm::vec3(x, 1.0f - y - height, 0.0f)) *
		scale;
	glUniformMatrix4fv(
		transformLocation, 1, GL_FALSE, glm::value_ptr(transform));
...
void Renderer::drawCube(RenderRequest& request) {
	...
	GLint projectionLocation =
		glGetUniformLocation(m_shaderColour, "u_Projection");
	...
	glUniform3f(
		colourLocation, request.Colour.x, request.Colour.y, request.Colour.z);
	glUniformMatrix4fv(viewLocation, 1, GL_FALSE, glm::value_ptr(view));
	...
	if (request.IsLight) {
		GLint lightPositionLocation =
			glGetUniformLocation(m_shaderColour, "u_LightPosition");
		GLint lightColourLocation =
			glGetUniformLocation(m_shaderColour, "u_LightColour");

		glUniform3f(lightPositionLocation, request.Position.x,
			request.Position.y, request.Position.z);
		glUniform3f(lightColourLocation, request.Colour.x, request.Colour.y,
			request.Colour.z);
	}
...
const unsigned int Renderer::createShader(
	...
	MATTENGINE_ASSERT(
		isCompiled == GL_TRUE, "Could not compile vertex shader.", NULL);
	...
	MATTENGINE_ASSERT(
		isCompiled == GL_TRUE, "Could not compile fragment shader.", NULL);

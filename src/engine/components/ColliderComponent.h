...
public:
	virtual void onCollision(CollisionType collisionType, Entity& staticEntity,
...
public:
	...
	void onCollision(CollisionType collisionType, Entity& staticEntity,
		Entity& movingEntity) override {

		TagComponent& movingTag = movingEntity.getComponent<TagComponent>();

		if (movingTag.Tag == "PowerUp")
			return;

		glm::vec2& velocity =
			movingEntity.getComponent<PhysicsComponent>().Velocity;
...
struct ColliderComponent {
	...
	template <typename T, typename... Args> void init(Args&&... args) {
